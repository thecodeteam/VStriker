...
public class EditConfigurationController {
	...
	private Button btnEditBack;
       ...
       @FXML private TextArea txtTestDesc;
       @FXML private NumericTextField txtObjSize;
       @FXML private NumericTextField txtNumOps;
       @FXML private NumericTextField txtNumTh;
       @FXML private NumericTextField txtNumofRetires;
       @FXML private ComboBox<ObjectSizeReportUnit> ddObjUnit;
       @FXML private CheckBox cbRandom;
       @FXML private CheckBox cbObjCreate;
       @FXML private NumericTextField txtPrctCreate;
       @FXML private CheckBox cbObjRead;
       @FXML private NumericTextField txtPrctRead;
       @FXML private CheckBox cbObjUpdate;
       @FXML private NumericTextField txtPrctUpdate;
       @FXML private CheckBox cbObjDelete;
       @FXML private NumericTextField txtPrctDelete;
	   @FXML private Pane paneRandom;
       @FXML private NumericTextField txtMinObjSize;
       @FXML private NumericTextField txtMaxObjSize;
       @FXML private NumericTextField txtPrctSmallSize;
       @FXML private NumericTextField txtPrctMedSize;
       @FXML private NumericTextField txtPrctLargeSize;
	...
	public void seTestEntity(vStrikerEntities.ConfigurationTemplate cfg, vStrikerEntities.TestConfiguration test, long l) {
		try {
	...
	public boolean ValidateRecord()
	{
		...
		if(txtTestName.getText().equals("")) isOK=ShowErrorMessage("Test Name is Required!");

		if(this.txtTestDesc.getText().equals("")) isOK=ShowErrorMessage("Test Desc is Required!");
		if(this.ddObjUnit.getValue()==null)isOK=ShowErrorMessage("Obj Report Unit is Required!");

		if(this.txtNumOps.getText().equals("")) isOK=ShowErrorMessage("Num of Opeations is Required!");
		if(this.txtNumTh.getText().equals("")) isOK=ShowErrorMessage("Num of Threads is Required!");
		if(this.txtNumofRetires.getText().equals("")) isOK=ShowErrorMessage("Num of Retires is Required!");
		if(this.txtObjSize.getText().equals("")) isOK=ShowErrorMessage("Test Obj Size  is Required!");
		if(!this.lblTotal.getText().equals("100%")) isOK=ShowErrorMessage("Total must be 100%!");
		
		if(!(this.cbAtmosHttp.isSelected() ||this.cbAtmosHttp.isSelected() ||
		this.cbSwiftHttp.isSelected() ||this.cbSwiftHttp.isSelected() ||
		this.cbS3Http.isSelected() ||this.cbS3Http.isSelected()))isOK=ShowErrorMessage("One API at least is required!");
	...
	public void LoadObjectUnitReport()
	{
		...
		try {
			...
			ObservableList<vStrikerEntities.ObjectSizeReportUnit> unit = vStrikerBizModel.ObjectSizeReportUnitBiz.ObjectSizeReportUnitSelectAll();
	...
	public void btnEditSaveClicked(ActionEvent event) {
		...
		if(ValidateRecord())
		{
			if(iOrginalID==-1) SaveTest(); else UpdateTest();
		}
   }
	public void SaveTest()	
	{
		try {
			...
			PrepareTestEntity();	       
			vStrikerBizModel.TestConfigurationBiz.TestConfigurationCreate(testcfg);

			System.out.println("New Cfg Test Id "+testcfg.getTestconfigurationId());
		...
		// Create Select API	
		 ...
		 for(ApiType a:apitypelist)
		 {
			 ...
			 selected.setTestConfiguration(testcfg);
			  if((a.getApiTypeName().equalsIgnoreCase("S3")) && (a.getApiTypeUrl().equalsIgnoreCase("http")) && (cbS3Http.isSelected())) AddSelectedAPI(a, selected); 
			  if((a.getApiTypeName().equalsIgnoreCase("S3")) && (a.getApiTypeUrl().equalsIgnoreCase("https"))&& (cbS3Https.isSelected())) AddSelectedAPI(a, selected);
			  if((a.getApiTypeName().equalsIgnoreCase("Swift")) && (a.getApiTypeUrl().equalsIgnoreCase("http"))&& (cbSwiftHttp.isSelected())) AddSelectedAPI(a, selected);
			  if((a.getApiTypeName().equalsIgnoreCase("Swift")) && (a.getApiTypeUrl().equalsIgnoreCase("https"))&& (cbSwiftHttps.isSelected())) AddSelectedAPI(a, selected);
			  if((a.getApiTypeName().equalsIgnoreCase("Atmos")) && (a.getApiTypeUrl().equalsIgnoreCase("http"))&& (cbAtmosHttp.isSelected())) AddSelectedAPI(a, selected);
			  if((a.getApiTypeName().equalsIgnoreCase("Atmos")) && (a.getApiTypeUrl().equalsIgnoreCase("https"))&& (cbAtmosHttps.isSelected())) AddSelectedAPI(a, selected);
			  
			  System.out.println("New Cfg Test Id "+selected.getApiSelectedId());
		 }
	

		 this.iOrginalID=testcfg.getTestconfigurationId();
		 
			JOptionPane.showConfirmDialog(null, "Test Configuration was inserted successfully!",  "VStriker",
				    JOptionPane.CLOSED_OPTION,JOptionPane.INFORMATION_MESSAGE);
		 
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			JOptionPane.showConfirmDialog(null, "Error during inserting test configuration data "+e.getMessage(),  "VStriker",
				    JOptionPane.CLOSED_OPTION,JOptionPane.ERROR_MESSAGE);
		}
	...
	public void UpdateTest() {
		...
		try {
			...
			vStrikerBizModel.TestConfigurationBiz.TestConfigurationUpdate(testcfg);
			System.out.println("New Cfg Test Id "+testcfg.getTestconfigurationId());
			
		 // Clean up old Selected API	
		  List<vStrikerEntities.VwApiSelectedDetail> apiSelList =vStrikerBizModel.ApiSelectedBiz.ApiSelectedSelectByTestID(testcfg.getTestconfigurationId());
		  for(vStrikerEntities.VwApiSelectedDetail a: apiSelList)
			 {
			  vStrikerBizModel.ApiSelectedBiz.ApiSelectDelete(a.getApiSelectedId());
			 }
		 ...
		 for(ApiType a:apitypelist)
		 {
			...
			JOptionPane.showConfirmDialog(null, "Test Configuration was updated successfully!",  "VStriker",
				    JOptionPane.CLOSED_OPTION,JOptionPane.INFORMATION_MESSAGE);
		 
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			JOptionPane.showConfirmDialog(null, "Error during updating test configuration data "+e.getMessage(),  "VStriker",
				    JOptionPane.CLOSED_OPTION,JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void onCheckTotal()
		{
			...
			try {c= Long.parseLong(txtPrctCreate.getText());}catch(Exception e){};
			try {r= Long.parseLong(txtPrctRead.getText());}catch(Exception e){};
			try {d= Long.parseLong(txtPrctDelete.getText());}catch(Exception e){};
			try {u= Long.parseLong(txtPrctUpdate.getText());}catch(Exception e){};
			...
			try
			{
				this.ClearForm();
					if(cfgtemp!=null)
					{
					       ...
					       this.cbRandom.setSelected(cfgtemp.getConfTempEnableRandSizeObject());
					       ...
					       if(this.cbObjCreate.isSelected()){this.txtPrctCreate.setText(String.valueOf(cfgtemp.getConfTempCreatePercent()));this.txtPrctCreate.setDisable(false);}
					       if(this.cbObjRead.isSelected()){ this.txtPrctRead.setText(String.valueOf(cfgtemp.getConfTempReadPercent()));this.txtPrctRead.setDisable(false);}
					       if(this.cbObjUpdate.isSelected()){this.txtPrctUpdate.setText(String.valueOf(cfgtemp.getConfTempUpdatePercent()));this.txtPrctUpdate.setDisable(false);}
					       if(this.cbObjDelete.isSelected()){this.txtPrctDelete.setText(String.valueOf(cfgtemp.getConfTempDeletePercent()));this.txtPrctDelete.setDisable(false);}
						 ...
						 List<vStrikerEntities.VwApiSelectedDetail> apiSelList =vStrikerBizModel.ApiSelectedBiz.ApiSelectedSelectByConfTempID(cfgtemp.getConfigurationTemplateId());
						 for(vStrikerEntities.VwApiSelectedDetail a: apiSelList)
						 {
					...
					if(testcfg!=null)
					{
					       ...
					       if(this.cbObjCreate.isSelected()){this.txtPrctCreate.setText(String.valueOf(testcfg.getCreatePercent()));this.txtPrctCreate.setDisable(false);}
					       if(this.cbObjRead.isSelected()){ this.txtPrctRead.setText(String.valueOf(testcfg.getReadPercent()));this.txtPrctRead.setDisable(false);}
					       if(this.cbObjUpdate.isSelected()){this.txtPrctUpdate.setText(String.valueOf(testcfg.getUpdatePercent()));this.txtPrctUpdate.setDisable(false);}
					       if(this.cbObjDelete.isSelected()){this.txtPrctDelete.setText(String.valueOf(testcfg.getDeletePercent()));this.txtPrctDelete.setDisable(false);}
